...
# 与之前一样，去掉图例
p[[18]] <- p[[17]] +
  ggplot2::theme(legend.position = "none",)  # 可能你的图片不需要去掉图例
...
# 与之前一样，字体大小。具体大小需要你根据自己图片的相对大小调整
p[[19]] <- p[[18]] +
  ggplot2::theme(axis.text = element_text(size = 12), # 坐标轴字体大小
                 axis.title = element_text(size = 14), # 坐标轴标题字体大小
                 strip.text = element_text(size = 14), # facet标题字体大小
                 plot.title = element_text(size = 16)) # 图片整体标题字体大小
...
---
class: center, middle
<span style="font-size: 60px;">11.2 Reproduce a Plot</span> <br>
...
---
## <h1 lang="en">df for Reproduce RT Plot</h1> 
...
df[[3]] <- df[[1]] %>%
  ...
  dplyr::summarise(RT_mean = mean(RT_ms),
                   ...
                   ACC_sd = sd(ACC)) %>%
  dplyr::ungroup()
names(df)[3] <- "Reproduce RT Plot"
```

---
## <h1 lang="en">Reproduce RT Plot, x = Identity, group = Vlance</h1> 
...
p[[25]] <- ggplot2::ggplot(data = df[[3]], aes(x = Identity, y = RT_mean, group = Valence, fill = Valence, color = Valence)) +
  ...
  theme_apa
names(p)[25] <- "stat_halfeye: RT, x = Identity, group = Vlance"; p[[25]]
...
p[[26]] <- ggplot2::ggplot(data = df[[3]], aes(x = Valence, y = RT_mean, group = Identity, fill = Identity, color = Identity)) +
  ...
  theme_apa
names(p)[26] <- "stat_halfeye: RT, x = Identity, group = Vlance"; p[[26]]
